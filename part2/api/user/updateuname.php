...
if (isset($_POST["username"])) {
    ...
    if ($_POST["username"] == $_SESSION["part2_username"]) {
        echo json_encode(
    ...
    try {
        $result = $user->updateUsername($_POST["username"], $_SESSION["part2_username"]);
        echo json_encode(
            array('message' => 'Username updated')
        );
        $_SESSION["part2_username"] = $_POST["username"];
        ...
        if (preg_match("/duplicate entry/i", $e->getMessage())) {
            ...
            error_log(print_r($e->getMessage(), true), 3, "/Applications/XAMPP/logs/php_error_log");
        }
    }
}
